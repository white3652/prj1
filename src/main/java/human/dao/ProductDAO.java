...
public class ProductDAO extends DBConnection {
	...
	public int insert(ProductDTO dto) {
		...
		try {
				if (dto.getOriginfile_name() != null) {
					String sql = "insert into admin_table (b_idx, m_idx,  title, content, writer, originfile_name, savefile_name)"
					...
					String sql = "insert into admin_table (b_idx, m_idx, writer, title, content)"
							+ " values(board_seq.nextval, ?, ?, ?, ?)";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, dto.getM_idx());
	...
	public ArrayList<ProductDTO> getBoards(){
		
		ArrayList<ProductDTO> boardList = new ArrayList<ProductDTO>();
		
		String sql = "select * from admin_table where del_or_not = 1 order by b_idx desc";
	...
	public ArrayList<ProductDTO> getBoards(SearchDTO sDto){
		ArrayList<ProductDTO> boardList = new ArrayList<ProductDTO>();
	...
	//조회수 업데이트
		public void updateRead_cnt(int b_idx) {
			...
			String sql = "update admin_table set read_cnt = (read_cnt + 1) where b_idx = ?";
		...
		//하나의 게시글 조회
		public ProductDTO getBoard(int b_idx){
			ProductDTO dto = null;
			
			String sql = "select * from admin_table where b_idx = ?";
		...
		public int update(ProductDTO dto) {
			...
			try {
				if(dto.getOriginfile_name() != null) {
					...
					String sql = "update admin_table set title=?, content=?, originfile_name=?, savefile_name=? "
							     + " where b_idx=?";
					...
					String sql = "update admin_table set title=?, content=? "
						     + " where b_idx=?";
		...
		public int delete(int b_idx) {
			...
			try {
				...
				String sql = "update admin_table set del_or_not=-1 "
					     + " where b_idx=?";
